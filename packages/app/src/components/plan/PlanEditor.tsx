...
export function PlanEditor({ todos: initialTodos, onExecute, onCancel, disabled }: PlanEditorProps) {
  ...
  return (
    <div className="plan-editor bg-secondary/30 border border-border rounded-lg p-3 my-2 max-h-[50vh] flex flex-col text-sm">
      {/* Header */}
      <div className="flex items-center justify-between mb-2 pb-1.5 border-b border-border flex-shrink-0">
        <div className="flex items-center gap-1.5">
          <Clock className="w-3.5 h-3.5 text-yellow-500" />
          <span className="font-medium text-xs">Plan Mode</span>
          <span className="text-[10px] text-muted-foreground">({totalSteps} steps)</span>
        </div>
          ...
          return (
            <div key={todo.step}>
              ...
              <div
                ...
                className={cn(
                  "flex items-center gap-1.5 p-1.5 rounded bg-background/50 border border-border/50",
              ...
              >
                ...
                {/* Step Number */}
                <span className="flex-shrink-0 w-5 h-5 flex items-center justify-center rounded-full bg-primary/10 text-primary text-[10px] font-medium">
                  {todo.step}
                </span>
                ...
                <input
                  ...
                  disabled={disabled}
                  className="flex-1 bg-transparent text-xs outline-none border-b border-transparent focus:border-primary/50 px-1 py-0"
                />
                ...
                {/* Subtask count badge */}
                {hasSubtasks && (
                  <span className="text-[10px] text-muted-foreground bg-secondary px-1 py-0 rounded">
                ...
                <div className="ml-6 mt-0.5 space-y-0.5">
                  {todo.subtasks!.map((subtask, subIndex) => (
                    <div
                      ...
                      className="flex items-center gap-1.5 p-1 rounded bg-background/30 border border-border/30"
                    >
                      {/* Step Number */}
                      <span className="flex-shrink-0 w-4 h-4 flex items-center justify-center rounded bg-muted text-muted-foreground text-[10px]">
                      ...
                      <input
                        ...
                        disabled={disabled}
                        className="flex-1 bg-transparent text-[11px] outline-none border-b border-transparent focus:border-primary/50 px-1 py-0"
                      />
...
export function PlanProgress({ completed, total, todos }: PlanProgressProps) {
  ...
  return (
    <div className="plan-progress bg-secondary/30 border border-border rounded-md p-2 my-1 max-h-[30vh] flex flex-col text-xs">
      ...
      {/* Progress Bar */}
      <div className="h-1 bg-secondary rounded-full overflow-hidden flex-shrink-0">
        <div
      ...
      <div className="mt-1 space-y-0 overflow-y-auto flex-1 min-h-0">
        {todos.map((todo) => (
          <div key={todo.step}>
            {/* Main task */}
            <div
            ...
            {todo.subtasks?.map((subtask) => (
              <div
                key={`${todo.step}.${subtask.step}`}
                className={cn(
                  "flex items-center gap-1.5 py-0 ml-3",
                  subtask.completed ? "text-muted-foreground line-through" : "text-foreground",
                )}
              >
                {subtask.completed ? (
                  <Check className="w-2 h-2 text-green-500 flex-shrink-0" />
                ) : (
                  <div className="w-2 h-2 rounded-full border border-muted-foreground/50 flex-shrink-0" />
                )}
