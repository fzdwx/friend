...
// SVG sprite URL - served from public folder
const SPRITE_URL = "/file-icons.svg";
...
// Helper functions from opencode
const basenameOf = (p: string) =>
  p.split("\\").join("/").split("/").filter(Boolean).pop() ?? "";
...
function chooseIconName(
  ...
  if (type === "directory") {
    for (const cand of folderNameVariants(baseLower)) {
      ...
      if (icon) {
        if (expanded && !icon.endsWith("Open")) {
          return icon + "Open";
        }
        return icon;
      }
...
export const FileIcon = memo(function FileIcon({
  ...
  const iconName = useMemo(
    () => chooseIconName(filename, "file", false),
    [filename]
  );
...
// Export for folder icons if needed
export const FolderIcon = memo(function FolderIcon({
  name,
  expanded = false,
  className,
  size = 16,
}: { name: string; expanded?: boolean } & Omit<FileIconProps, "filename">) {
  const iconName = useMemo(
    () => chooseIconName(name, "directory", expanded),
    [name, expanded]
  );
